...
Three frozen encoders are compared. The reference is DOFA~\cite{xiong2024}, a vision
transformer pretrained across optical, multispectral and radar sources whose input
projection is generated by a hypernetwork from the central wavelength of each channel,
which is what lets radar and optical share one extractor; we use its ViT-B/16 weights
(768 dimensions per sensor, 1{,}536 fused) and its ViT-L/16 weights (1{,}024 per sensor,
2{,}048 fused), DOFA-B and DOFA-L below. The optical channels are
declared at their band centers (0.49, 0.56, 0.665, 0.842 and 1.61 $\mu$m) and the radar
pair at the marker the checkpoint was trained with. The third encoder is Copernicus-FM
base~\cite{wang2025cfm}, a ViT-B/16 whose spectral hypernetwork takes each channel's
central wavelength and bandwidth and which additionally conditions on the location, date
and footprint of every window; it receives the window's coordinates and area from the
composite grid and the middle of the composited year as its date. A fourth set of
...
\paragraph{Comparing feature extractors} Every experiment is run on the base and the large
feature extractor with the same head, protocol, seeds and splits; every table reports both and
...
\textbf{Feature extractor} (Table~\ref{tab:feature extractor}). Under grouped cross-validation over the
