...
cities the map reaches a within-municipality rank correlation of $\rho = 0.20$ with a base and $0.23$ with a large DOFA backbone, 84\% and 77\% of what the same features
attain under full tract supervision ($0.23$ and $0.30$); under grouped cross-validation
over all 138 cities the larger backbone improves the map by $0.03$ [$0.01$, $0.05$].
...
the base and the large backbone. Against free global grids the
...
interval spans 1.7 to 1.9 of the five levels. Data, frozen features of both backbones,
...
\item Test maps reach 84\% (base backbone) and 77\% (large) of the fully supervised bound
...
produces: the token-level deprivation score from municipal aggregates alone (base backbone), on the same scale. \textbf{e}, The same prediction averaged to the tract, the unit at which every
...
supervised bound within municipalities with the base backbone and 77\% with the large one, whose maps are higher in absolute terms, and the curve along the number of
...
supports ($\rho = 0.251$ with context and $0.202$ without for the base backbone; $0.318$ and
$0.260$ for the large backbone). The two other axes of the
...
2{,}048 fused), called the base and the large backbone below. The optical channels are
...
distance can be separated. The head was tuned on the base backbone and reused unchanged
on the others.
...
backbone is split after its tenth block: the frozen prefix is applied once to every
...
backbone learning rate is $10^{-5}$, with $10^{-4}$ as a sensitivity run, against
$2 \times 10^{-4}$ for the head; three seeds each.

\paragraph{Comparing backbones} Every experiment is run on the base and the large
backbone with the same head, protocol, seeds and splits; every table reports both and
every figure draws the large backbone hatched or with hollow markers. Fourteen cities
are too few to separate backbones, so they are compared by grouped cross-validation
...
backbones is bootstrapped on the same city draws. Copernicus-FM enters that comparison
...
appended to the 1{,}536 dimensions of the base backbone encoder; this ablation was run on
the base backbone only. Two fusions are compared on the full pool,
three seeds each, against the base backbone head without extras: early, the columns enter the projection
...
backbones, and kept where the WorldCover built fraction is $\geq 0.10$. A bag is a
...
the same for both backbones. Nested subsets of 50 and 100 Colombian bags give the
supply axis abroad.
...
reports the fourteen held-out test cities with validation-selected models, for the base backbone
and the large backbone in that order; validation values are shown beside them as the selection
...
the backbones are compared under cross-validation below and why it is the point
estimates, consistent across splits and backbones, that the title claims. The larger backbone raises the upper bound by $0.06$ and the map by $0.02$ to
$0.03$ on both splits, so the aggregates recover a smaller share of a richer
representation. The test curve of the base backbone reproduces the rise through 400 aggregates
almost exactly and flattens at the top; the large backbone is higher at every point of the curve
...
$0.168$ on test with the base backbone, and from $0.238$ to $0.134$ and from $0.223$ to $0.182$
with the large backbone; a single national aggregate erases it ($0.009$ and $0.010$ validation,
...
against 0.747 optical). With the large backbone the optical sensor alone reaches what the fused
the base backbone model reaches. Sensor content dominates ground sampling distance, with
...
mean over three seeds against held-out tract grades, for the base backbone (solid, filled) and
the large backbone (dashed or hatched, hollow); blue, the 14 validation cities used for
...
split ($\rho = 0.251$ validation and $0.233$ test for the base backbone; $0.318$ and $0.295$
for the large backbone, lighter). \textbf{a}, Agreement as a function of how many municipal
...
\begin{tabular}{rcccc}
\toprule
 & \multicolumn{2}{c}{Base backbone} & \multicolumn{2}{c}{Large backbone}\\
...
prediction of each backbone averaged to the tract, all on one color scale over the
...
138 cities (363 municipalities), the large backbone reaches a within-municipality $\rho$ of
$0.230$ [$0.203$, $0.257$] per token and $0.294$ [$0.259$, $0.328$] per AGEB, against
$0.203$ [$0.170$, $0.234$] and $0.252$ [$0.213$, $0.292$] for the base backbone; the paired
...
indistinguishable from the base backbone ($0.197$ per token; difference $-0.006$ [$-0.029$,
...
cross-validated value of the base backbone ($0.203$) sits between its validation ($0.227$) and
test ($0.196$) values.
...
cities: the fused the large backbone vectors ($0.226$), the fused the base backbone vectors ($0.196$),
the thirteen land-cover columns of WorldCover fractions and NDVI with no learned
representation ($0.219$, indistinguishable from the base backbone on test and far below it on
validation, $0.150$), the optical vectors of either backbone alone ($0.179$ and
...
the three backbones are within $0.04$ of one another on any one split, and only the
...
\midrule
Large backbone, optical and radar fused & 2048 & 0.242 & 0.226 & 0.230 [0.203, 0.257]\\
Large backbone, optical 10 m & 1024 & 0.235 & 0.207 & \\
Large backbone, optical degraded to 20 m & 1024 & 0.217 & 0.191 & \\
Large backbone, radar & 1024 & 0.022 & 0.016 & \\
Base backbone, optical and radar fused & 1536 & 0.227 & 0.196 & 0.203 [0.170, 0.234]\\
Base backbone, optical 10 m & 768 & 0.183 & 0.179 & \\
Base backbone, optical degraded to 20 m & 768 & 0.171 & 0.129 & \\
Base backbone, radar & 768 & 0.006 & 0.084 & \\
...
Base backbone fused plus GHSL and lights, early fusion & 1540 & 0.225 & -- & \\
\bottomrule
\end{tabular}
\label{tab:features}
\end{table}
...
municipal labels well (quadratic-weighted $\kappa = 0.62$ with the base backbone, $0.68$ with
the large backbone) and produces a map at chance (AUROC 0.463 and 0.391;
...
\textbf{a}, Map quality by model family on identical features and labels, the base backbone
solid and the large backbone hatched; the first three families are measured on the national-set
...
folds of the national set), filled the base backbone and hollow the large backbone, the only criterion
...
AUROC it would seem to proxy. \textbf{c}, The backbone comparison of Table~3 city by
city: under grouped cross-validation over the 138 cities, the mean within-municipality
$\rho$ each city obtains with the large backbone against the base one; 84 of 138 cities
...
directly to the token scores that form the map: AUROC 0.761 (base) and 0.782
(large) under the same features, bags and labels on that national-set pool, and
...
the base backbone; $0.242$ against $0.242$ and $0.253$ against $0.226$ with the large backbone).
...
within-municipality map quality on the base backbone and $+0.25$ on the large backbone, and $+0.62$ and
...
standard error near $0.27$, so the estimates are coarse and the two backbones' values
...
$\rho$ on either backbone ($0.15$ to $0.19$ and $0.18$ to $0.21$). Whether this extends beyond settings where the bag label is an average of
...
reaches $\rho = 0.35$ [$0.29$, $0.44$] with the base backbone and $0.35$ [$0.31$,
$0.41$] with the large one against $0.14$ [$0.05$, $0.27$] for the index on validation,
...
backbone ($\Delta\rho = +0.22$ [$+0.08$, $+0.29$] and $+0.22$ [$+0.10$, $+0.29$]) and
...
Base backbone, winning 16 of 29 municipalities; $+0.01$ [$-0.10$, $+0.11$] for the large backbone,
...
validation and $0.257 \pm 0.146$ on test with the base backbone, and at $0.341 \pm 0.053$ and
$0.313 \pm 0.113$ with the large backbone, in every case at least as high as against its own
...
with the base backbone, and 31--38\% and 42--64\% with the large backbone, with no crossover on test
for either (Fig.~\ref{fig:validation}c). At the tightest validation budget (5\%), where
errors at the top of the ranking are most costly, the base backbone loses to the aggregate and
the large backbone closes 4\% of the gap. The map is also not an administrative
artifact. Crossing a municipal border where deprivation does not change moves it by
$+0.019$ [$-0.002$, $+0.155$] with the base backbone and $-0.003$ [$-0.033$, $+0.126$] with
the large backbone, and tract borders inside municipalities are crossed smoothly ($+0.001$ and
...
purple in \textbf{d}, which belongs to neither split; the base backbone solid and filled, the large backbone hatched or hollow. \textbf{a}, Within-municipality agreement on the identical AGEB
...
backbone and $0.27$ [$0.15$, $0.37$] with the large one; GHSL residential population
...
splits ($+0.04$ and $-0.04$ with the base backbone, $+0.04$ and $+0.00$ with the large
...
the base backbone and hollow the large backbone, with city-clustered bootstrap intervals; only gridded population separates from the map on
...
validation cities, the base backbone solid and the large backbone dashed. The marginal rule falls below
nominal in ten of fourteen cities with the base backbone and eleven with the large backbone; the
...
with the base backbone, 0.818 and 0.827 with the large one). Orientation was fixed before
...
against the map's 0.829 with the base backbone and 0.827 with the large backbone
...
backbone (seeds $0.23$, $0.28$, $0.24$; AUROC 0.66 for the most deprived quartile) and
$0.17$ with the large one ($0.21$, $0.16$, $0.14$), against $0.31$ and $0.52$ for the
...
give $0.33$ and $0.24$ with the base backbone and $0.18$ and $0.19$ with the large one:
...
backbone ($0.12$, $0.13$, $0.13$; pooled $0.34$; AUROC 0.64) and $0.16$ [$0.10$, $0.21$]
with the large one, against $0.10$ [$0.05$, $0.15$] and $0.17$ [$0.12$, $0.22$]
...
The Mexican model transfers better with the larger backbone in both countries without
...
for both backbones on the same bags. Within-municipality Spearman against the fine truth,
...
earlier zero-shot protocol, without the built-token filter and with the base backbone,
...
\toprule
Country & Backbone & Method & Seeds & Mean [95\%] & Pooled $\rho$ & AUROC\\
\midrule
...
$0.044$ [$-0.011$, $0.084$] on test with the base backbone, and at $0.115$ [$0.045$, $0.187$] and
$0.079$ [$0.030$, $0.120$] with the large backbone: a signal too weak to act on. Acting on it
...
discarded on validation and $0.317$ to $0.327$ on test with the base backbone, $0.382$ to
$0.346$ and $0.316$ to $0.274$ with the large backbone, every point inside every other point's
...
half-width of 1.29 grades under the marginal rule with the base backbone (1.30 with the large backbone),
and that rule undercovers: 0.869 overall and below nominal in 10 of the 14 test cities,
worst at La Paz (0.768), with the base backbone; 0.857, 11 cities and Hidalgo (0.718) with the large backbone. Pricing in that cities are not exchangeable with one another, by taking each
...
\toprule
Backbone & Rule & Half-width (grades) & Coverage & Worst city (coverage) & Cities below nominal\\
\midrule
Base & marginal & 1.29 & 0.869 & La Paz (0.768) & 10 of 14\\
Base & clustered by city & 1.68 & 0.953 & Madera (0.870) & 1 of 14\\
Large & marginal & 1.30 & 0.857 & Hidalgo (0.718) & 11 of 14\\
Large & clustered by city & 1.85 & 0.963 & La Paz (0.899) & 1 of 14\\
\bottomrule
\end{tabular}
\label{tab:conformal}
\end{table}
...
score rises with the true grade for either backbone and the distributions of adjacent
...
the test cities, base backbone solid and large backbone hatched (boxes: quartiles;
whiskers: 5th to 95th percentile; $n$ tracts per grade below). \textbf{b}, Median tract
score by grade in each of the fourteen test cities (gray, base backbone) and over all
...
tracts with the base backbone, $-0.07$ with the large backbone), Ciudad del Carmen, Campeche ($-0.27$ and
...
Guerrero's median spread (0.101 with the base backbone) sits at the middle of the fourteen.

\begin{table}[htbp]
\centering
\caption{The fourteen test cities ranked by the within-municipality correlation their map
achieves with the base backbone, worst first, with the large backbone value beside it. Tract-mean
...
with the base backbone, $0.318$ and $0.295$ with the large backbone: sufficient to order neighborhoods,
too coarse to resolve individual blocks. Agreement depends strongly on tract size with
either backbone: $\rho = 0.26$ for tracts covered by 1--3 tokens ($0.18$ with the large backbone), rising to $0.43$ at 20--49
...
\toprule
Tokens per tract & Tracts & Median population & Municipalities & Base backbone & Large backbone\\
\midrule
...
what the base backbone features support and 76--77\% of what the large backbone features support,
...
both without adaptation, better with the larger backbone. The two countries hold
...
validation with the base backbone, 77\% and 76\% with the large backbone, whose maps are higher in
