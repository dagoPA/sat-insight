...
over all tokens. Brackets: 95\% percentile bootstrap resampling cities.
...
\midrule
DOFA-B & 0.203 [0.170, 0.234] & 0.252 [0.213, 0.292] & 0.802\\
DOFA-L & 0.230 [0.203, 0.257] & 0.294 [0.259, 0.328] & 0.804\\
Copernicus-FM base & 0.197 [0.170, 0.225] & 0.252 [0.221, 0.281] & 0.799\\
\midrule
...
radar vectors ($0.084$ and $0.016$). Letting the extractor move does not help either:
with the last two blocks of DOFA-B trained together with the head, the map reaches
$0.209$ (s.d. $0.023$ over seeds) on validation and $0.169$ ($0.020$) on test at an
encoder learning rate of $10^{-5}$, and $0.167$ and $0.151$ at $10^{-4}$, against
$0.227$ and $0.185$ for the frozen extractor with the same retrained head, while the bag
error it is trained on improves ($0.091$ against $0.098$ on validation). The aggregates
carry enough signal to fit the municipal figure better and not enough to teach the
representation where the deprivation is, the same dissociation that attention pooling
and the selection sweep show. Copernicus-FM base reaches $0.212$ on validation,
...
where it was run. Dimensions are per token. The fine-tuned rows train the last two of
DOFA-B's twelve blocks with the head (Methods); their s.d. over seeds is $0.02$ or less.}
\small
\begin{tabular}{llccc}
\toprule
...
DOFA-B, last two blocks fine-tuned, encoder lr $10^{-5}$ & 1536 & 0.209 & 0.169 & \\
DOFA-B, last two blocks fine-tuned, encoder lr $10^{-4}$ & 1536 & 0.167 & 0.151 & \\
\bottomrule
\end{tabular}
\label{tab:features}
\end{table}
...
the map reaches a quadratic-weighted Cohen's $\kappa$ of $0.39$ on the 2{,}811 test
tracts with either feature extractor ($0.42$ and $0.45$ on validation) and an accuracy
of $0.40$ and $0.43$ over five grades; the AUROC of the four cumulative thresholds
averages $0.79$ pooled over the split and $0.64$ inside municipalities. The pooled
figures are higher than the within-municipality ones for the reason that runs through
the paper: they include the between-city gradient, the half of the problem the municipal
aggregate already answers. For reference, the phase-one classifier trained on the tract
labels themselves reached $\kappa = 0.54$ on the same task; the map reaches $0.39$
having never seen one.

\begin{table}[htbp]
\centering
\caption{The map read as a five-grade classifier. Tract-mean scores of the seed
ensemble; the grade is assigned by the isotonic score-to-grade map fitted on the
validation tracts. $\kappa$ is quadratic-weighted Cohen's kappa and accuracy is over the
five grades, both pooled over the split's tracts; AUROC is the mean over the four
cumulative thresholds (grade at least $k$), pooled over the split and averaged over the
22 municipalities with at least twenty tracts.}
\small
...
splits ($+0.04$ and $-0.04$ with DOFA-B, $+0.04$ and $+0.00$ with DOFA-L). A 1 km light product, free and requiring no model, orders tracts inside a Mexican
...
block strata as truth over 12{,}738 of them, so the within-municipality value is the
correlation inside that one municipality; the parenthesis is the s.d. over seeds (the
unadapted model has one). Brazil: 48
...
\midrule
Colombia & DOFA-B & Mexican, unadapted & 0.311 & 0.705\\
Colombia & DOFA-B & local aggregates, 50 bags & 0.329 (0.066) & 0.701\\
Colombia & DOFA-B & local aggregates, 100 bags & 0.242 (0.038) & 0.655\\
Colombia & DOFA-B & local aggregates, 153 bags & 0.254 (0.027) & 0.657\\
Colombia & DOFA-B & Mexican, fine-tuned & 0.315 (0.048) & 0.696\\
Colombia & DOFA-B & oracle & 0.747 (0.010) & 0.903\\
Colombia & DOFA-L & Mexican, unadapted & 0.520 & 0.806\\
Colombia & DOFA-L & local aggregates, 50 bags & 0.183 (0.039) & 0.610\\
Colombia & DOFA-L & local aggregates, 100 bags & 0.189 (0.046) & 0.615\\
Colombia & DOFA-L & local aggregates, 153 bags & 0.168 (0.037) & 0.602\\
Colombia & DOFA-L & Mexican, fine-tuned & 0.414 (0.019) & 0.748\\
Colombia & DOFA-L & oracle & 0.793 (0.005) & 0.918\\
\midrule
Brazil & DOFA-B & Mexican, unadapted & 0.099 [0.048, 0.148] & 0.648\\
Brazil & DOFA-B & local aggregates, 48 bags & 0.123 [0.075, 0.171] & 0.640\\
Brazil & DOFA-B & Mexican, fine-tuned & 0.078 [0.026, 0.129] & 0.633\\
Brazil & DOFA-B & oracle & 0.242 [0.193, 0.290] & 0.704\\
Brazil & DOFA-L & Mexican, unadapted & 0.171 [0.120, 0.223] & 0.689\\
Brazil & DOFA-L & local aggregates, 48 bags & 0.155 [0.103, 0.210] & 0.694\\
Brazil & DOFA-L & Mexican, fine-tuned & 0.185 [0.136, 0.236] & 0.689\\
Brazil & DOFA-L & oracle & 0.285 [0.230, 0.337] & 0.736\\
\bottomrule
\end{tabular}}
\label{tab:transfer}
\end{table}
...
0.31$ with DOFA-B, $0.52$ with DOFA-L over the city's tokens). In Rio de Janeiro the
agreement is weaker ($0.04$ and $0.15$, mean over the box's municipalities): the maps
put the west and the northern suburbs above the south zone, as the income truth does,
but the favelas inside the wealthy south are smoothed over at 160 m and the gradient
inside each municipality is faint, which is the informality result of the zero-shot
check seen on the map.
...
all 138 cities, while fine-tuning the extractor on the aggregates lowers the bag error
and not the map: the representation is one limit on the map and the supply of labels is
