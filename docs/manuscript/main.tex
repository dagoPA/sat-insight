...
with AGEB boundaries overlaid; tokens of 160 m tile the box. \textbf{c}, The held-out
tract truth, every token painted with the GRS grade of the AGEB it falls in.
\textbf{d}, What the model produces: the token-level deprivation score from municipal
aggregates alone, on the same scale. Tapachula sits at the validation median
($\rho = 0.47$), chosen as a median-performing, representative example.}
\label{fig:design}
\end{figure}
