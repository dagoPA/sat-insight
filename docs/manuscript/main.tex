...
token of every city was re-encoded with two further frozen backbones: DOFA large (1024
dimensions per sensor, 2048 fused) and Copernicus-FM base~\cite{wang2025cfm}, which
additionally takes the location, date and footprint of each window. Fourteen cities are
too few to separate backbones, so they are compared by grouped cross-validation over
all 138 cities of the partition. The cities are dealt into five folds stratified as the
...
bootstrapped on the same city draws.
...
[$0.29$, $1.83$]; fourteen cities bound the precision, and it is the point estimate,
...
with fourteen cities the difference is invisible. The cross-validated value of DOFA base
($0.203$) sits between its validation ($0.227$) and test ($0.196$) values. All other
results in this paper use DOFA base, the backbone the protocol was developed on; the
cross-validated gain of DOFA large is the most direct improvement to the predictor these
experiments found.
