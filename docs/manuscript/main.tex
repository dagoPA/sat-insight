...
cities: 84\% of what the same features attain there under full tract supervision
($0.23$; 90\% on the validation cities used for model selection). More aggregates
...
2019~\cite{ibge2019}, Bogot\'a's district stratification~\cite{bogota}. For training
abroad: Colombia's municipal multidimensional poverty rate (DANE, census
2018)~\cite{dane_ipm} with the MGN 2023 municipal boundaries~\cite{mgn2023}, and
Brazil's municipal mean and tract median monthly income of the household head (IBGE,
census 2022)~\cite{ibge2022} with the 2022 census tract geometries.
...
size and deprivation, no city split across sets. Hyperparameters are selected on
...
Comparisons are paired by municipality over three seeds; every interval on a
within-municipality mean is a city-clustered bootstrap (2{,}000 resamples), and curve
error bars are s.d. over seeds.
...
aggregates (projection and scaler loaded, scoring row initialised as the mean of the four
...
\noindent The same four products, plus the eleven WorldCover class fractions and the
NDVI mean and dispersion of the composite, are also tested as inputs. Each is
reprojected onto the 10 m grid and averaged inside every 160 m token (population and
built surface as $\log(1 + x)$), giving four GHSL and light columns and thirteen
WorldCover columns appended to the 1{,}536 encoder dimensions. Two fusions are
compared on the full pool, three seeds each, against the canonical head: early, the
columns enter the projection with the encoder vector; late, they bypass it and enter the
scoring layer beside the hidden representation, each with a weight of its own per
threshold. The oracle is re-measured with the same inputs, standardised, since a
nighttime-light column near 45 on an unstandardised scale would dominate the first
layer; its no-extras reference is re-run under the same standardisation, and differences
are paired by municipality with a city-clustered bootstrap.
...
reports the fourteen held-out test cities with validation-selected models; validation
values are shown beside them as the selection benchmark. \textbf{Quantity} (Fig.~\ref{fig:curve}a):
...
a property of the method or of Mexico (Fig.~\ref{fig:validation}d, Supplementary
Table~8). Trained from scratch on Colombia's municipal poverty rate over 35 bags, the
...
full tract supervision the four GHSL and light columns lift the standardised oracle from
...
and $0.226$ (Supplementary Table~7). Information that a tract-supervised head
...
layer and an honest grade interval spans 1.7 of the five levels. The design repeats
...
selection (error bars, s.d. over seeds); red, the 14 held-out test cities, drawn
without a spread; dashed and dotted lines, the fully supervised oracle upper bound on each split
