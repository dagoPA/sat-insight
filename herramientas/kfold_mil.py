...
Usage: kfold_mil.py [folds] [epochs] [objective] [entropy]
...
for k, held_out in enumerate(folds):
    ...
    pd.concat(histories, ignore_index=True).to_csv(
        f"data/mil_history_{OBJECTIVE}_{ENTROPY}.csv", index=False
    )
    pd.DataFrame(results).to_csv(f"data/mil_kfold_{OBJECTIVE}_{ENTROPY}.csv", index=False)
