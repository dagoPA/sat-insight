...
def with_truth(table: pd.DataFrame) -> pd.DataFrame:
    ...
    truth = pd.read_parquet("data/grs_ageb_2020.parquet")[["cvegeo", "ordinal", "poblacion"]]
...
def conformal(calibration: pd.DataFrame, evaluation: pd.DataFrame, isotonic) -> pd.DataFrame:
    ...
    pd.concat(city_rows, ignore_index=True).to_csv(
        "data/incertidumbre_cobertura_ciudad.csv", index=False
    )
    return pd.DataFrame(rows)
...
def main() -> None:
    validation = with_truth(per_ageb("data/predicciones_val.parquet"))
    test = with_truth(per_ageb("data/predicciones_test.parquet"))
    ...
    pd.DataFrame(rows).to_csv("data/incertidumbre_spread.csv", index=False)
    ...
    for split, frame in (("val", validation), ("test", test)):
        ...
        curves.append(curve)
    pd.concat(curves, ignore_index=True).to_csv("data/incertidumbre_selectiva.csv", index=False)

    conformal(validation, test, isotonic).to_csv("data/incertidumbre_conformal.csv", index=False)

    ranking = audit(test)
    ranking.to_csv("data/auditoria_ciudades_test.csv", index=False)
