...
Usage: figura_validez.py <destination.pdf>
"""
...
def _incumbent(ax, book) -> None:
    ...
    validation = pd.read_csv("data/incumbente_pareado.csv").iloc[0]
    test = pd.read_csv("data/incumbente_pareado_test.csv").iloc[0]
...
def _replication(ax, book) -> None:
    ...
    own_validation = pd.read_csv("data/curva_supervision.csv").query("bags == 771").spearman_within
    ...
    conapo_validation = pd.read_csv("data/replicacion_imu.csv").iloc[0]
    conapo_test = pd.read_csv("data/replicacion_imu_test.csv").iloc[0]
    agrees(
...
def _targeting(ax, book) -> None:
    ...
    for split, path, expected in (
        ("validation", "data/focalizacion.csv", book["targeting_pooled"]),
        ("test", "data/focalizacion_test.csv", book["test"]["targeting"]),
    ):
...
def _transfer(ax, book) -> None:
    ...
    table = pd.read_csv("data/transferencia_zeroshot.csv").set_index("city")
