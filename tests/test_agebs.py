...
def test_el_orden_ordinal_es_de_menor_a_mayor_rezago():
    ...
    assert ORDINAL["Muy alto"] == len(GRADES) - 1
...
def test_cada_ciudad_declara_un_municipio_de_su_entidad():
    for clave, ciudad in CITIES.items():
        assert ciudad.key == clave
        assert len(ciudad.state) == 2
        assert len(ciudad.municipality) == 5
        assert ciudad.municipality.startswith(ciudad.state)


def test_el_resumen_cuenta_todas_las_clases_aunque_falten():
    resumen = grade_summary(tabla(["Bajo", "Bajo", "Alto"]))
    assert list(resumen.index) == list(GRADES)
...
def test_la_mancha_descarta_el_satelite_lejano():
    ...
    mancha = _connected_mass(agebs, "07101", bridge_m=2500)
