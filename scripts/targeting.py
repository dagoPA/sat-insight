...
HIGH = 3  # ordinal of the high deprivation grade
...
def reached(order: pd.DataFrame, budget: float) -> float:
    ...
    """
    cap = order.population.sum() * budget
    population = order.population.to_numpy(dtype="float64")
...
def main() -> None:
    ...
    # heterogeneity is part of the result: the average must not hide the cities where
    # the map loses to the aggregate
    ...
    multi = pd.read_parquet("data/predictions_val.parquet").groupby("city").municipality.nunique()
