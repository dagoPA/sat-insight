...
Usage: national_composites.py [index total]
"""
...
def main() -> int:
    ...
    for n, key in enumerate(keys, start=1):
        ...
        try:
            ...
            rows.append({"key": key, "municipality": beyond[key].municipality, "agebs": len(agebs)})
            pd.DataFrame(rows).to_csv(
                f"data/national_composites_{label.replace(' ', '_')}.csv", index=False
            )
