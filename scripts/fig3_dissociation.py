...
def _families(ax, book) -> None:
    ...
    swept = agrees(
        pd.read_csv("data/supervision_curve.csv").query("bags == 771").auroc_high.mean(),
        book["curve"]["771"]["auroc_high"],
        name="swept head map AUROC",
    )
    oracle_seeds = [
        pd.read_csv(path).auroc.iloc[0]
        # the canonical oracle files carry no split or extras tag; the fusion ablations
        # and the test column share the prefix and must stay out of the mean
        for path in sorted(Path("data").glob("oracle_expanded_r1_s[0-9].csv"))
    ]
    oracle = agrees(
...
def _blindness(ax, book) -> None:
    ...
    sweep = pd.read_csv("data/llp_sweep_r1.csv")
