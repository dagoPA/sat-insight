...
OUT = (
    ...
    if RADIUS != 1 or TAG
    else "data/supervision_curve.csv"
)
...
def main() -> None:
    ...
    pool = load_split(train_cities, SENSOR, fuse=FUSE, extras=EXTRAS)
    val_bags = load_split(val_cities, SENSOR, fuse=FUSE, extras=EXTRAS)
