...
standardises the inputs with training statistics, as the weakly supervised head does;
without it a product on a large scale, nighttime lights sit near 45, would dominate
the first layer, so a fused run and its no-extras reference must both carry the flag.
...
def main() -> None:
    ...
    print(
        f"{POOL} pool · radius {RADIUS} · extras {EXTRAS} · std {STANDARDIZE} · "
        f"{total_labelled} labelled training instances · {train_bags[0].instances.shape[1]} dims",
        flush=True,
    )
    ...
    if STANDARDIZE:
        # the same 200-per-bag sample the weakly supervised head standardises with
    ...
    # paired by municipality: the upper bound with and without context is compared on the same bags
    within = []
    with torch.inference_mode():
    ...
    ).to_csv(f"data/oracle_{POOL}_r{RADIUS}_s{SEED}_{EVAL_SPLIT}{TAG}.csv", index=False)
    pd.DataFrame(within, columns=["municipality", "rho"]).assign(radius=RADIUS, seed=SEED).to_csv(
        f"data/oracle_bags_{POOL}_r{RADIUS}_s{SEED}_{EVAL_SPLIT}{TAG}.csv", index=False
    )
